...
const MOCK_FIELDS = ['negotiation_status', 'created_at'];
const MOCK_OPERATORS = ['eq', 'neq', 'gt', 'gte', 'lt', 'lte', 'is_null'];

export const FilterBuilder = ({ config, setConfig }: FilterBuilderProps) => {
  const addFilter = () => {
      ...
      <div className="space-y-2">
        {config.filters.map(filter => (
          <div key={filter.id} className="grid grid-cols-12 gap-2 items-center p-2 bg-gray-50 rounded-md">
            <select value={filter.field} onChange={e => updateFilter(filter.id, { field: e.target.value })} className="col-span-4 p-2 border rounded-md text-sm">
